...
- 交付物（作业PDF P2）
  - 源代码（15分）—— 实际就是提示词 + 调用LLM的脚本
  - 完整对话日志（4次迭代，带时间戳）（15分）
  - 报告（附录模板，20分）
...
1. **Individual Reflection**（每人一段，中文或英文均可）：
   1. 成员A：遇到的问题（如模型输出格式不遵循步骤）及解决方案（如强化prompt中的“必须”）
   2. 成员B：如何确保迭代间的依赖（如手动复制前次输出的元素名）
   3. 成员C：报告整合中如何保证一致性
2. **贡献表**（作业PDF第7页）：
   1. 
